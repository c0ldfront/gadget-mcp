			...
			messages: [
				userMessage(
					[
						...
						"",
						"The tool's first content block is an imperative top-line instruction that tells you EXACTLY what to do next. OBEY IT without re-asking the user — the user has already made their choice at the wizard's preview step. Specifically:",
						'- `action: "dispatch-now"` → call your Task tool NOW with `cwd` set to the project path and the composed `prompt` as the subagent task. Do not re-confirm.',
						'- `action: "plan-first"` → dispatch a planning Task NOW with cwd = project path, the composed `prompt`, and an added constraint that it MUST produce a blueprint only (no code, no filesystem writes). Do not re-confirm.',
						'- `action: "sampled"` → surface the `sampled.text` the host model returned.',
						'- `action: "returned"` → show the composed `prompt` in a code block and ask the user what they want to do.',
						'- `action: "cancelled"` → acknowledge and stop.',
						"",
